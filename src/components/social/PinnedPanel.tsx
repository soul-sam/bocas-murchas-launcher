...
export function PinnedPanel() {
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col border-l border-[#1a1a1a] bg-[#0D0D0D]">
        ...
        {loadingPinned && pinned.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {pinned.map((message) => (
              <div
                key={message.id}
                className="group rounded-brutal border border-[#1a1a1a] bg-void-light/30 p-2 transition-colors hover:border-burn/40"
              >
                <button
                  type="button"
                ...
                {outOfReach === message.id && (
                  <p className="mt-1 font-mono text-[9px] uppercase tracking-widest text-burn">
                ...
                {isAdmin && (
                  <button
                    ...
                    title="Desafixar"
                    className="mt-1 flex items-center gap-1 font-mono text-[9px] uppercase tracking-widest text-muted-foreground opacity-0 transition-opacity hover:text-destructive group-hover:opacity-100"
                  >
